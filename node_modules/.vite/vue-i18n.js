...
function vTDirective(i18n) {
  function process2(binding) {
    const { instance, modifiers, value } = binding;
    if (!instance || !instance.$) {
      throw createI18nError(I18nErrorCodes.UNEXPECTED_ERROR);
    }
    const composer = getComposer$2(i18n, instance.$);
    if (process2.env.NODE_ENV !== "production" && modifiers.preserve) {
  ...
  const register = (el, binding) => {
    const [textContent, composer] = process2(binding);
  ...
  return {
    ...
    beforeUpdate: update,
    getSSRProps: (binding) => {
      const [textContent] = process2(binding);
      return { textContent };
    }
  };
}
